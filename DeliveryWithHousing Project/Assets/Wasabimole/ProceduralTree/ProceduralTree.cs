    ...
    public void GenerateTree()
    {
        ...
        void Branch(Quaternion quaternion, Vector3 position, int lastRingVertexIndex, float radius, float texCoordV)
        {
            ...
            if (radius < MinimumRadius || vertexList.Count + NumberOfSides >= MaxNumVertices) // End branch if reached minimum radius, or ran out of vertices
            {
				if (placeIco)
				{
					Transform item = Instantiate(ico,position + transform.position,quaternion) as Transform;
					item.name = "Branch Break";
			}
		...
		//========Code for leaves at each segment======================

		//	Transform leaves = Instantiate(leafPrefab) as Transform;
		//	leaves.position = transform.position+ position;
