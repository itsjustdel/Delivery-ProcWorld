...
public class csTriangulation : MonoBehaviour
{
    ...
    public class Triangulation
    {
        ...
        public IEnumerator Calculate(JoinCellAndVerge joinCellAndVerge)
        {
         //   Debug.Log("calcs");
            if (this.vertices.Count == 0)
            {
                Debug.Log("Cs tri vertices = 0");
                yield break;
            }
            Vector3[] result;
            int i;
            ...
            {
                Debug.Log("Cs tri vertices < 3");
                result = new Vector3[vertices.Count];
                for (i = 0; i < vertices.Count; i++)
                {
                    result[i] = vertices[i].pos;
                }
                //joinFieldAndSkirt.tri = result;
                foreach (Vector3 v3 in result)
                    joinCellAndVerge.tri.Add(v3);
                yield break;
            }
        ...
        //    foreach (Vector3 v3 in joinFieldAndSkirt.tri)
        //        tempList.Add(v3);

            foreach (Vector3 v3 in result)
                joinCellAndVerge.tri.Add(v3);

            joinCellAndVerge.CreateMesh();

           // joinFieldAndSkirt.tri = tempList;
...
/*
        public void Calculate(JoinCellAndVerge joinCellAndVerge)
        {
