    ...
    void CreateTerrain()
    {
        ...
        for (int zCount = 0; zCount < maxHeight; zCount++)
        {
            ...
            for (int xCount = 0; xCount < maxLength; xCount++)
            {
                ...
                heights[zCount, xCount] = height; //original
        ...
        //del
        //adjust height     //SETTINGS MUST SHIFT HEIGHT 1 - and plus terrain y over 100
        //set terrain object to collider's position
        //terrain builds from bottom left - bounds min
        //minus the y size of the bounds if using bottum up. Line 102, builds heights from the bottom
        terrainObject.transform.position = bounds.min - (bounds.size.y*Vector3.up);
