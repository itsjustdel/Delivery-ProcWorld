...
//	public Pathfinder pathFinder;
	public WaypointPathfinder waypointPathFinder;
   
	void Start()
	{
		if (PathType == PathfinderType.GridBased)
		{
//		pathFinder = gameObject.AddComponent<Pathfinder>();//trying to create a map for each agent
		}
		else if (PathType == PathfinderType.WaypointBased)
		{
//		waypointPathFinder = gameObject.GetComponent<WaypointPathfinder>();//trying to create a map for each agent
		}
	}
...
//			pathFinder.InsertInQueue(startPosition, endPosition, SetList);
        ...
        {
//			waypointPathFinder = gameObject.GetComponent<WaypointPathfinder>();
//           WaypointPathfinder.Instance.InsertInQueue(startPosition, endPosition, SetList); 
			waypointPathFinder.InsertInQueue(startPosition, endPosition, SetList);
        }
    }
