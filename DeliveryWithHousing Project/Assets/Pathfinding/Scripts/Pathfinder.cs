...
public class Pathfinder : MonoBehaviour 
{
	...
	//Del
	public bool addTransformPosition;
	public CurveInjector curveInj;
	//public BRoadPlotter bRoadPlotter;
	public BRoadMesher bRoadMesher;
	public GameObject TerrainPrefab;
	public bool canReplaceTerrain;
    //Singleton
	...
	void OnEnable () 
    {
		...
		{
		CreateMap();//trying to create non singleton for each agent??
		}
	}
		...
		private void CreateMap()
		{
			...
			for (int i = 0; i < height; i++)
			{
				for (int j = 0; j < width; j++)
				{
					...
					RaycastHit[] hit; 
					LayerMask lM = LayerMask.GetMask("TerrainBase","Road");
					if (CheckFullTileSize)
					{
						hit = Physics.SphereCastAll(new Vector3(x, HighestPoint, y), Tilesize / 2, Vector3.down, dist,lM);
					}
					else
					{
						hit = Physics.SphereCastAll(new Vector3(x, HighestPoint, y), Tilesize / 16, Vector3.down, dist,lM);
					}
					bool free = true;
	...
	//	yield return new WaitForFixedUpdate();
		GameObject terrainPrefab = Instantiate(TerrainPrefab) as GameObject;
		terrainPrefab.transform.position = this.transform.position;
		terrainPrefab.name = "Terrain Base";
		yield break;
	}
        ...
        if (startNode != null)
        {
            ...
            while (!endLoop)
            {
                ...
                if (sortedOpenList.Count == 0)
                {
					...
					//create new Terrain// either that or change pathfinder values??
					
					//GameObject.FindWithTag("TerrainBase").AddComponent<TerrainDiamondSquare>();

					if (canReplaceTerrain){

						StartCoroutine("ReplaceTerrain");



					GameObject terrain  = GameObject.FindGameObjectWithTag("TerrainBase");
            ...
            while (currentNode.parent != null)
            {
				...
				//this Del written piece of codes passes the values found by PathFinder in to the array used
				//for meshing splines. CurveInj is a piece of crap and bRoadmesher is the new thing. #swag
				if (curveInj != null){

					curveInj.list.Add(new Vector3(currentNode.xCoord, currentNode.yCoord, currentNode.zCoord));
				}else if (curveInj== null &&bRoadMesher!=null){
					bRoadMesher.pathFull.Add(new Vector3(currentNode.xCoord, currentNode.yCoord, currentNode.zCoord));
				}
            }
			...
			//curveInj.list = returnPath;  //feeds curveinjector script the list
			//curveInj.add = true;
