...
public class WaypointPathfinder : MonoBehaviour 
{
    ...
    public List<Vector3> returnPath;// = new List<Vector3>();
    ...
    public bool forNormalWaypoints;
    public bool forMiddleWaypoints;
    public bool forVoronoiWaypoints;
    //public bool  makeMapLeft;
    ...
    public void MakeNewMap()
	{
		...
		GameObject[] M = new GameObject[0];
	
		if (!forHex && forNormalWaypoints)
		{
			M = GameObject.FindGameObjectsWithTag("Waypoint");
		}

        else if(!forHex && forMiddleWaypoints)
        {
            M = GameObject.FindGameObjectsWithTag("Waypoint");
        }
		...
		{
			M = GameObject.FindGameObjectsWithTag("HexWaypoint");
		}
        else if(forVoronoiWaypoints)
            M = GameObject.FindGameObjectsWithTag("VoronoiWaypoint");

        //change from collider to trigger
        foreach (GameObject b in M)
		{
		//	b.GetComponent<BoxCollider>().enabled = false;

            if(!forVoronoiWaypoints)//using this if atm, dont know when it used!
		...
		{
            if (M[i].transform.parent.name == "Waypoints")
            {
              //  M[i].transform.parent = transform;
            }
	...
	//			gameObject.GetComponent<WaypointPathfinder>().Map[currentNode.ID].isActive = false;
	//			MakeNewMap();
			...
			if(!forHex) //if this is being used for agents pathfinding and not forbuilding the world
			{
				//let agent behaviour know we have found a path
				if (returnPath.Count > 1)
				{
//					gameObject.GetComponent<AgentBehaviour>().pathfinderFoundRoute = true;
					//using path.count > 2 on agent behaviour script now
				}
			}
			return returnPath;
    ...
    public void SetStartAndEndNode(Vector3 start, Vector3 end)
    {
        startNode = FindClosestNode(start);
        //GameObject c = GameObject.CreatePrimitive(PrimitiveType.Cube);
       // c.transform.transform.position = startNode.position;
       // c.name = "node start";
        endNode = FindClosestNode(end);

       // c = GameObject.CreatePrimitive(PrimitiveType.Cube);
       // c.transform.transform.position = endNode.position;
       // c.name = "node end";
    }

    public WaypointListNode FindClosestNode(Vector3 pos)
    {
        Stopwatch a = new Stopwatch();
        a.Start();
        //UnityEngine.Debug.Log(Map.Length);
        int ID = -1;
