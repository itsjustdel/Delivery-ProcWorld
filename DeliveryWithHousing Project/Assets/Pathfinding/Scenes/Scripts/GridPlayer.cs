...
public class GridPlayer : Pathfinding
{
	...
	public Vector3 target ; //is asigned by placegridplayer script
	...
	void Start()
	{
		...
		if(forRingRoad)
		{
		//Find furthest Point script is upto branches in heirarchy, grab the points it worked out for start and end
		transform.position = this.transform.parent.parent.GetComponent<FindFurthestCorner>().p1;
		target = this.transform.parent.parent.GetComponent<FindFurthestCorner>().p2;

		}
		// finds where the terrain is on the Y and starts from there (200 max look)
		RaycastHit hit;
		if (Physics.Raycast(transform.position, -Vector3.up, out hit, 200.0F))
			transform.position = hit.point;

		//Pathfinder path;
		//path.curveInj = GameObject.FindGameObjectWithTag ("Road Dresser").GetComponent<CurveInjector> ();
